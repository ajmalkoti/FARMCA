...
clc;    clear all;      close all;  %BP.PhiE
setup_code(pwd)                                                      

% STEP: 1) load, select, derive parameters.
% load all the parameters, data for given zone of interest
[Logs,PRho,PGR,PDepth, PSim,PShRad,KB] = fun_param_testing('balol183_4.txt','fig_InputLogs')  ;         % plotfig_logs(logs, 'fig_InputLogs')            
...
[BP2,IdxRep]= fun_remove_repetitions(BP,PShRad);              % plotfig_removed_repetitions(BP,BP2); 
...
[BPPoreFract, PShRad2] = fun_find_parameters_fractal(BP2, PShRad);

% skip the values forwhich ratio check ==0 
[BPPoreFract,BP3, IdxRep] = fun_remove_nonfractal(BPPoreFract,BP2,IdxRep); 
close('all') 

% STEP: 5) Error analysis (partial): This is required to find the NRun using max porosity (else not required).  
PSim2 = fun_error_analysis_on_max_porosity(BP2,BPPoreFract,PSim,KB);

% Error analysis (full run: for all porosities) 
[Dt,K,NBinSeries,KStat,SimLBD,tau] = fun_perm_estimation(IdxRep.LbdUniIdx,BP2,BPPoreFract,PSim2,KB);
save(['OUTPUT',filesep,'state_fullrun.mat']);             % load(['OUTPUT',filesep,'state_fullrun.mat']); 

%% Fill back for the duplicate coloumns 

%%SimLBD
[KStat, Dt, stat, Kfull] = fun_fill_duplicate2(BPPoreFract, Dt, KStat, BP, BP2, IdxRep); 
%R,          Dt,temp,LBD,    KStat, BP, BP2, index);
