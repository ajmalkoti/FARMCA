...
function  [n_elem_bin_series2,  LogKbin_mean, err, KUBound, KLBound, median_K, tau]=fn_corr_time2(K,cp)
%K=log(K);
dim=single(size(K));
bin_level= 0:floor(log2(dim(1)));

for i=0:(max(bin_level)-1)
    n_elem_bin_series2(i+1) = floor(dim(1)/(2^i)/2) *2;
end
...
for j=0:max(bin_level)-1
    K_bin=K(1:n_elem_bin_series2(j+1),:);                  % k binned series at zeroth level
    ...
    BS_max= n_elem_bin_series2(j+1)/nB_min;                                               %bin size of last bin
